        ...
        function* (sessionID: SessionID) {
          ...
          while (true) {
            ...
            if (
              ...
              // 同时也作为门禁：如果代码未审计，强制运行 Inspector
              const enableInspector = agent?.options?.enableInspector !== false && agent?.name === "superpower"
              const enableGatekeeping = agent?.options?.enableGatekeeping !== false && agent?.name === "superpower"

              // 检查是否需要运行 Inspector
              let needsInspection = enableInspector
              let gatekeepingBlocked = false

              if (enableGatekeeping && enableInspector) {
                const currentSession = yield* sessions.get(sessionID)
                const lastEditAt = currentSession.lastEditAt
                const lastAuditPassAt = currentSession.lastAuditPassAt

                // 如果有编辑且审计通过时间早于编辑时间，必须运行 Inspector
                if (lastEditAt && (!lastAuditPassAt || lastAuditPassAt < lastEditAt)) {
                  log.info("gatekeeping: code not audited, forcing inspector", {
                    sessionID,
                    lastEditAt,
                    lastAuditPassAt,
                  })
                  gatekeepingBlocked = true
                  yield* sessions.updatePart({
                    id: PartID.ascending(),
                    messageID: lastUser.id,
                    sessionID,
...
- 最后编辑时间: ${lastEditAt ? new Date(lastEditAt).toISOString() : "无"}
- 最后审计通过时间: ${lastAuditPassAt ? new Date(lastAuditPassAt).toISOString() : "从未通过审计"}

**正在自动启动 Inspector 进行 4 轮审计验证...**
...
**必须首先调用 skill: superpowers:verification-before-completion 获取验证指令！**

然后按照 skill 的要求执行完整验证：

## 验证优先级
1. plan 是否实现 - 检查计划文件，逐一验证每项要求
2. README 是否正确 - 运行文档中的命令，验证与代码一致
3. 代码能否跑通 - 安装、测试、构建、运行
4. 语法是否正确 - 类型检查、Lint
                  ...
                  Effect.gen(function* () {
                    try {
                      ...
                      const existing = yield* Effect.promise(() =>
                        Filesystem.exists(memoryPath).then((exists) => (exists ? Filesystem.readText(memoryPath) : "")),
                      )
...
`
                  ...
                  void Effect.runPromise(Effect.ignore(Notify.send({
                    title: `🔴 Inspector 检查失败`,
                    message: `**项目**: ${Instance.directory.split("/").pop()}\n**迭代**: #${step}\n**状态**: ${isRegressed ? "质量退化，已回滚" : "检查未通过"}\n\n请检查 memory.md 了解详情。`,
                    status: "error",
                  })))
...
`
                ...
                // Update lastAuditPassAt for gatekeeping - 4轮检查全部通过
                yield* sessions.setAuditPass({ sessionID })

                // 发送飞书通知 - 检查通过
                void Effect.runPromise(Effect.ignore(Notify.send({
                  title: `🟢 Inspector 检查通过`,
                  message: `**项目**: ${Instance.directory.split("/").pop()}\n**迭代**: #${step}\n**状态**: 4 轮检查全部通过\n\n工作摘要: ${workSummary.slice(0, 200)}...`,
                  status: "success",
                })))
...
迭代 #${step} 记录已写入 \`memory.md\`。
                ...
                // 如果是门禁触发的 Inspector，通过后直接完成（不再次进入优化循环）
                if (gatekeepingBlocked) {
                  log.info("gatekeeping-triggered inspector passed, allowing completion", { sessionID })
                  // 继续执行 autoCommit 和后续逻辑
                }
              }
            ...
            const outcome: "break" | "continue" = yield* Effect.onExit(
              Effect.gen(function* () {
                ...
                if (finished && !handle.message.error) {
                  ...
                  const title = sessionInfo?.title && !Session.isDefaultTitle(sessionInfo.title)
                    ? sessionInfo.title
                    : "任务完成"
                  void Effect.runPromise(Effect.ignore(Notify.send({
                    title: `✅ ${title}`,
                    message: `**会话**: ${sessionID}\n**状态**: 任务已完成\n\n请前往 TUI 查看结果。`,
                    status: "success",
                  })))
                } else if (finished && handle.message.error) {
                  // 发送飞书通知 - 任务错误
                  const errorMsg = handle.message.error.message || "未知错误"
                  void Effect.runPromise(Effect.ignore(Notify.send({
                    title: `❌ 任务执行错误`,
                    message: `**会话**: ${sessionID}\n**错误**: ${errorMsg.slice(0, 200)}\n\n请前往 TUI 查看详情。`,
                    status: "error",
                  })))
                }
